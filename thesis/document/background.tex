\section{The Evolution of Data Models}
	...
	\subsection{Semi-structured Data and Query Languages:  1990 -- 2010}
		...
		Interestingly enough, \gls{xml} has proven to be inefficient as a data description format\todo{citation?}.  Nonetheless, a semi-structured data description language is highly sought after for message passing in large-scale software systems.  Modern Web services are built on the concept of RESTful \gls{api} \cite{restful-11}, with semi-structured data message passing.  In order to minimize network overhead, \gls{xml}-based message passing has been replaced by more efficient data encoding standards such as \gls{json} \cite{json}.
