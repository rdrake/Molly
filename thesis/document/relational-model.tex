...
\section{Relational Model}
	...
	Relations are typically represented as tables, as shown in \cref{tbl:course-relation}.
	...
	\subsection{Schema Group}
		...
		\label{def:schema-graph}
			If we view relations as vertices, and foreign key constraints as edges, a database \(\db\) can be viewed as a schema graph, \(\sgraph{}\), formally defined as
			\begin{align}
		...
		\begin{ex}
			...
			\begin{figure}
				...
				\caption{\gls{fk} constraints on schema in \vref{tbl:schema}}
		...
		The relational data model is particularly powerful for analytic queries.  One can formulate the following analytic queries in a query language known as \gls{sql}.
			
		\begin{ex}
			Given the schema graph in \cref{fig:schema-graph}, use \gls{sql} to find all section CRNs for the subject titled ``Community Development \& Policy Studies.''  The \gls{sql} query in \cref{fig:query-section-crns} results in \cref{tbl:query-section-crns-output}.
