...
Error: column code is not unique
			...
			With the uniqueness of named tuples guaranteed (as demonstrated in \cref{ex:unique-constraint}), we must ensure that any named tuples that are referenced actually exist.  If they do not, the database must not permit the operation to continue.  Doing so would lead to dangling references.
