     ...
     {:T :entity
      ...
      :sql    (->
                (join
                  (->
                    (table :sections)
                    (project [[:id :as :sec_id] :actual :campus :capacity :credits :levels :registration_start :registration_end :semester :sec_code :sec_number :year :course]))
                  (->
                    (join
                      (->
                        (table :schedules)
                      ...
                      (->
                        (table :teaches)
                        (project [:position :teaches.schedule_id :teaches.instructor_id]))
